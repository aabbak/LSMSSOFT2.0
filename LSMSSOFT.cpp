/* The LSMSSOFT computes a gravimetric geoid by using the KTH method and additive corrections.
 * 
 ...
 *
 * Authors    : R. Alpay ABBAK, Aydin USTUN
 * Address    : Konya Technical University, Geomatics Engineering Dept, Campus, Konya, TURKEY
 * E-Mail     : raabbak@ktun.edu.tr, ydnstn@yahoo.com 
 *
 * Reference paper for the program:
 * Abbak, RA. and Ustun A. (2015), A software package for computing a gravimetric geoid model
 * by the KTH method, Earth Science Informatics, 8(1), 255-265. DOI: 10.1007/s12145-014-0149-3.
 *
 * Compilation of the program on Linux: 
 * g++ LSMSSOFT.cpp matris.cpp levelell.cpp -o LSMSSOFT
 *
 * Execution of the program with our sample data: 
 * ./LSMSSOFT -Gitg-grace2010s.gfc -Aanomaly.xyz -Eelevation.xyz 
 * 
...
{//** DEFINITIONS ***************************************************************************//
	...
	double 	 MinPhi=45.0125;		// minimum latitude of target area
	double 	 MaxPhi=46.9875;		// maximum latitude of target area
	double 	 MinLam=2.0125;			// minimum longitude of target area
	double 	 MaxLam=3.9875;			// maximum longitude of target area
	double 	 PhiInt=0.025;			// grid size in latitude direction
	double 	 LamInt=0.025;			// grid size in longitude direction
	...
	double approxN=0.0;			// approximate undulation
	...
	double   geoid=0.0;			// estimated geoid height by the KTH method
	...
	for(phi=MinPhi-1.0;phi<=MaxPhi+1.0;phi=phi+PhiInt)
   	{
		...
		for(lam=MinLam-1.0;lam<=MaxLam+1.0;lam=lam+LamInt)
		{
			...
			for(i=in-framePhi;i<=in+framePhi;i++) 
			{
				for(j=jn-frameLam;j<=jn+frameLam;j++)
				{
					...
					y=abs(j-jn); 		// position on the compartment
					if(psi(x,y)<0.008728) 	// capsize=0.5 degree for gravity gradient
						tot=tot+(dg(i,j)-dg(in,jn))/l_0(x,y)*cos(lati(i));
				}
			}
    	...
    	framePhi=round(psi0*rho/PhiInt);	// vertical limit of compartment
    	frameLam=round(acos((cos(psi0)-pow(sin(MaxPhi/rho),2))/(pow(cos(MaxPhi/rho),2)))*rho/LamInt);// horizontal limit of compartment
	...
	for(phi=MinPhi;phi<=MaxPhi;phi=phi+PhiInt)
   	{
		...
		for(lam=MinLam;lam<=MaxLam;lam=lam+LamInt)
		{
			...
			ell=((0.0036-0.0109*sin(slat)*sin(slat))*dg(in,jn)+0.005*approxN*cos(slat)*cos(slat))*QM0;
			DWC1=H(in,jn)*(dg(in,jn)/gamma+3.0*approxN/(R+H(in,jn))-Dgr(in,jn)*H(in,jn)/2.0/gamma);
			for(n=2;n<=M;n++)
			...
			if(s) printf("%13.8f%13.8f%10.4f%8.4f%8.4f%8.4f%8.4f%10.4f\n",phi,lam,approxN,top,atm,dwc,ell,geoid);
			else printf("%13.8f%13.8f%10.4f\n",phi,lam,geoid);
			atm=DWC2=DWC3=stokes=dpot=gdst=modif=0.0;
		}
	}
	return 0;
}// ** FINISH *******************************************************************************//
...
void HELP()
{
	...
	fprintf(stderr,"     LSMSSOFT -G[model<file>] -A[anomaly<file>] -E[elevation<file>] -D[density<file>] -M[<value>]\n");
	...
	fprintf(stderr,"                default: 45.0125/46.9875/2.0125/3.9875\n\n");
	fprintf(stderr,"     -I<value>  intervals of the grid (LatInterval/LonInterval)\n");
	fprintf(stderr,"                default: 0.025/0.025 (90x90 arc-seconds)\n\n");
	fprintf(stderr,"     -S         prints all segments (lat, lon, Ñ, top, atm, dwc, ell, N), respectively.\n\n");
	fprintf(stderr,"     -H         prints this help\n\n");
	fprintf(stderr,"AUTHORS:        Dr. R. Alpay ABBAK & Dr. Aydın ÜSTÜN\n\n");
}
